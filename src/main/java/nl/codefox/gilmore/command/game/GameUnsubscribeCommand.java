...
public class GameUnsubscribeCommand extends GilmoreCommand {
    ...
    public void process(String command, String[] args, TextChannel channel, User author, MessageReceivedEvent event) {
        ...
        if (!GameCommand.gameExists(name)) {
            channel.sendMessage(String.format("[%s] `This game doesn't exist. Try !game list to get a list of available games", author.getAsMention(), name));
        } else {
            Game game = GameCommand.getGame(name);

            if (!game.getInterestedUsers().contains(author.getId())) {
                channel.sendMessage(String.format("[%s] `You are not subscribed to the game '%s'`", author.getAsMention(), name));
                return;
            }

            game.removeUser(author.getId());
            GilmoreDatabase.removeSubscriber(name, author.getId());
            channel.sendMessage(String.format("[%s] `You are now unsubscribed from the game '%s'`", author.getAsMention(), name));
        }
