...
public class DeletePermissionCommand extends GilmoreCommand {
    public DeletePermissionCommand() {
        super("", "!permission delete [role] ![command] [subcommand]", 2, 10, Permission.MANAGE_SERVER, "!permission add", "!perm add");
    }
                ...
                for (GilmoreCommand gilmoreCommand : Gilmore.getCommandListener().getCommands()) {
                    if (gilmoreCommand.getAliases().contains(finalCommand)) {
                        for (String alias : gilmoreCommand.getAliases()) {
                            if (!PermissionsCommand.removeCommandPermission(alias, role)) {
                                Message message = channel.sendMessage(String.format("[%s} The command %s failed to delete %s", author.getAsMention(), alias, role.getName()));
                                new MessageDeleter(message);
                            }
                        }
                    }
                }
                Message message = channel.sendMessage(String.format("[%s} The command %s no longer requires the role %s", author.getAsMention(), finalCommand, role.getName()));
                new MessageDeleter(message);
            } else {
